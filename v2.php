...
foreach($all_user->data->rank as $user){
	...
	foreach($pets as $pet){
		if($pet->avatar_file->name=='twilightsparkle.png'){
			$petid=$pet->id;
		$petname=$pet->name;
		...
		for($i=0;$i<$babacount;$i++){//清理baba
			...
			if(json_decode($r->getBody())->data->code==0){
				echo $echohead.'成功清理'.$user->nick.'之宠'.$petname.'的1个baba'.PHP_EOL;
			}else{
				echo $echohead.'给'.$user->nick.'之宠'.$petname.'清理baba失败（次数上限）'.PHP_EOL;
			}
		}
		$foodamount=$foodamount/20;//喂食
		$num=($user->uin==QQNUM)?5:3;
		for($i=$foodamount;$i<$num;$i++){
			if($food_count>=FOOD_LIMIT){
				echo $echohead.'喂食次数已达到设定的LIMIT'.PHP_EOL;
				break;
			}
			...
			if(json_decode($r->getBody())->data->code==0){
				echo $echohead.'成功给'.$user->nick.'之宠'.$petname.'喂食1次'.PHP_EOL;
				$food_count++;
			}else{
				echo $echohead.'给'.$user->nick.'之宠'.$petname.'喂食失败（次数上限）'.PHP_EOL;
			}
			}
			break;
		}
